...
import { Customers, CustomersDocument } from '../../entities/customer.schema';
import { AuthService } from '../auth/auth.service';
import { Tokens, TokensDocument } from '../../entities/token.schema';
...
export class CustomersService {
  ...
  async login(
    ...
    res.cookie('refreshToken', refreshToken, {
      ...
      secure: true, // Chỉ cho phép cookie qua HTTPS nếu TRUE
      maxAge: 7 * 24 * 60 * 60 * 1000, // Hết hạn sau 7 ngày
      sameSite: 'none', // Cho phép cookie được gửi qua các miền khác
      path: '/',
    });
  ...
  async logout(id: string) {
    const token = await this.tokenModel.findOne({ customer_id: new Types.ObjectId(id) });
    if (token) {
      await this.authService.setTokenBlackList(token?.refreshToken);
      await this.authService.setTokenBlackList(token?.accessToken);
      // Xoá bản ghi token khỏi DB
      await this.tokenModel.deleteMany({ customer_id: new Types.ObjectId(id) });
    }
  }
